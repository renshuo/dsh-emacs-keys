/**
 * dsh-emacs-key-binding — Cordis Plugin (v5, config-driven)
 *
 * Takes over DSH Web keyboard shortcuts and reconfigures them with
 * Emacs-style bindings.
 *
 * v5: keybindings live in a CONFIG FILE, not in code.
 *  - emacs-keybindings.json (session workspace) holds a FEATURE TREE:
 *    categories[] → { name, context, bindings: { actionId: { key, label } } }.
 *    `key` uses Emacs notation (C=Ctrl, M=Alt, S=Shift, SPC=space; a
 *    space-separated sequence like "C-x k" binds a prefix chord).
 *  - `context` selects the dispatch layer: region (any focus), global
 *    (outside inputs), editing (inside inputs), prefix:<key> (sequence),
 *    prefix-start (sequence starters).
 *  - The Host half reads the file through the fs service and serves it to
 *    the Client half via a Package-private RPC; the Client falls back to
 *    an embedded default tree when the file is missing or invalid.
 *  - C-x C-r hot-reloads the config (also re-mounts the settings page).
 *  - The settings page renders the ACTIVE config tree and its source.
 *
 * Lifecycle: ctx.effect() owns the listener; the returned disposer
 * removes it on stop / update / undefine.
 */
